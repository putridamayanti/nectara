...
export default function LeaveType({ leaveType }) {
    const { url } = usePage();
    ...
    const submitDelete = () => {
        return Inertia.delete(`/app/leave-type/${selectedItems.join(',')}`)
    };
    ...
    return (
        <>
                ...
                <CardContent>
                    ...
                    <TableContainer>
                        <Table sx={{ whiteSpace: 'nowrap' }}>
                            ...
                            <TableBody>
                                {leaveType.data.length > 0 ? leaveType?.data?.map((row, i) => (
                                    <TableRow key={i}>
                                        ...
                                        <TableCell align="right">
                                            <Tooltip title="Edit">
                                                <Link href={`/app/leave-type/${row.id}/edit`}>
                                                    <IconButton>
                    ...
                    <Stack direction="row" paddingTop={3} justifyContent="end">
                        <Pagination
                            color="secondary"
                            count={leaveType?.last_page ?? 1}
                            page={leaveType.current_page}
            ...
            <Link href="/app/leave-type/create">
                <Fab
                    color="primary"
                    aria-label="add"
